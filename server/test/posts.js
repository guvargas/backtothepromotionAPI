...
test('Should get posts', async function () {
	...
	const response = await request(`${process.env.URL_DEFAULT}/posts`, 'get');
...
test.only('Should save a post', async function () {
	...
	const response = await request(`${process.env.URL_DEFAULT}/posts`, 'post', data);
...
test('Should not save a post', async function () {
	const data = { acao: generate(), descricao: generate() };
	const response1 = await request(`${process.env.URL_DEFAULT}/posts`, 'post', data);
	const response2 = await request(`${process.env.URL_DEFAULT}/posts`, 'post', data);
...
test('Should update a post', async function () {
	...
	const response = await request(`${process.env.URL_DEFAULT}/posts/${post.id}`, 'put', post);
...
test('Should delete a post', async function () {
	const post = await postsService.savePost({ acao: generate(), descricao: generate() });
	const response = await request(`${process.env.URL_DEFAULT}/posts/${post.id}`, 'delete');
	expect(response.status).toBe(204);
